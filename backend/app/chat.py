...
async def get_device_positions(
    ...
    # Include only essential fields to reduce data size
    if resp and isinstance(resp, list):
        cleaned_data = [{k: v for k, v in pos.items() if k in ('fixTime', 'latitude', 'longitude', 'speed')} for pos in resp]
        if len(cleaned_data) > MAX_ROWS_THRESHOLD:
            #file_url = _save_large_response(cleaned_data, "positions")
            #print("file_url", file_url)
            return 'TOO_MANY_POINTS'
        print(f"cleaned: {len(cleaned_data)} items, first 3: {cleaned_data[:3]}")
        return cleaned_data
